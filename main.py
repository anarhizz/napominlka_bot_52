...
def set_reminder_name(message):
    ...
    bot.send_message(message.chat.id,'enter the date and time when you want to receive a reminder in the format YYYY-MM-DD hh:mm:ss.' )
...
def reminder_set(message,user_data):
    try:
        ...
        if delta.total_seconds()<=0:
            ...
            ['reminder_name']
            bot.send_message(message.chat.id,'reminder "{}" is set to {}.'.format(reminder_name,reminder_time))
            reminder_timer=threading.Timer(delta.total_seconds(),send_reminder,[message.chat.id,reminder_name])
            reminder_timer.start()
    except ValueError:
        bot.send_message(message.chat.id,'you entered the wrong date and time format,try again.')
def send_reminder(chat_id,reminder_name):
    bot.send_message(chat_id,'Time to get your reminder "{}"!'.format(reminder_name))
@bot.message_handler(func=lambda message:True)
def handle_all_message(message):
    bot.send_message(message.chat.id, "i dont understand what you're saying . tO create a reminder,type /reminder")
